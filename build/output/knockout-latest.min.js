...
ko.jsonExpressionRewriting=function(){function e(d,f){return d.replace(a,function(g,i){return f[i]})}var a=/\[ko_token_(\d+)\]/g,b=/^[\_$a-z][\_$a-z]*(\[.*?\])*(\.[\_$a-z][\_$a-z]*(\[.*?\])*)*$/i,c=["true","false"];return{parseJson:function(d){d=ko.utils.stringTrim(d);if(d.length<3)return{};for(var f=[],g=null,i,h=d.charAt(0)=="{"?1:0;h<d.length;h++){var j=d.charAt(h);if(g===null)switch(j){case '"':case "'":case "/":g=h;i=j;break;case "{":g=h;i="}";break;case "[":g=h;i="]";break}else if(j==i){j=d.substring(g,
...
ko.jsonExpressionRewriting.parseJson(d),g=[];for(var i in f){var h=f[i],j;j=h;j=ko.utils.arrayIndexOf(c,ko.utils.stringTrim(j).toLowerCase())>=0?false:j.match(b)!==null;if(j){g.length>0&&g.push(", ");g.push(i+" : function(__ko_value) { "+h+" = __ko_value; }")}}if(g.length>0){f=g.join("");d=d+", '_ko_property_writers' : { "+f+" } "}return d}}}();
...
1][g]===undefined?Number.MAX_VALUE:f[h-1][g]+1,f[h][g-1]===undefined?Number.MAX_VALUE:f[h][g-1]+1)}e=e;a=a;b=e.length;c=a.length;d=[];g=f[c][b];if(g===undefined)f=null;else{for(;b>0||c>0;){i=f[c][b];j=c>0?f[c-1][b]:g+1;k=b>0?f[c][b-1]:g+1;h=c>0&&b>0?f[c-1][b-1]:g+1;if(j===undefined||j<i-1)j=g+1;if(k===undefined||k<i-1)k=g+1;if(h<i-1)h=g+1;if(j<=k&&j<h){d.push({status:"added",value:a[c-1]});c--}else{if(k<j&&k<h)d.push({status:"deleted",value:e[b-1]});else{d.push({status:"retained",value:e[b-1]});c--}b--}}f=
d.reverse()}return f}}})();
...
a.insertBefore(o,a.firstChild):a.appendChild(o);else l.nextSibling?a.insertBefore(o,l.nextSibling):a.appendChild(o);l=o}break}ko.utils.arrayForEach(k,function(m){ko.utils.domData.cleanNodeAndDescendants(m)});c=false;if(!f){d.afterAdd&&d.afterAdd(i);if(d.beforeRemove){d.beforeRemove(k);c=true}}c||ko.utils.arrayForEach(k,function(m){m.parentNode&&m.parentNode.removeChild(m)});ko.utils.domData.set(a,"setDomNodeChildrenFromArrayMapping_lastMappingResult",b)}})();
